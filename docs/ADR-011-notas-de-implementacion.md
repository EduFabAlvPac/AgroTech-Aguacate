...
**(RESUELTO en §5undecies — el login con Google ya pide el segundo factor; el de Campesino no aplica.) Gap original: el challenge de MFA solo corre en el provider de credenciales (email+contraseña).** El login con Google (`GoogleProvider`) y el de celular sin contraseña (`telefono-campesino`, modo Campesino) NO piden código MFA aunque la cuenta lo tenga activo — ninguna cuenta privilegiada usa esos dos caminos hoy (Google es exclusivo del perfil "Otro rol" para cuentas ya existentes sin autoregistro; Campesino nunca es Super Admin/OWNER). Extender el challenge a esos dos providers es sencillo si algún día hace falta (mismo patrón: revisar `mfaHabilitado` antes de completar `resolverClaimsSesion()`), pero no tiene ningún caso real que lo ejercite todavía.
...
## 5undecies. MFA también en el login con Google (cierra el hueco del Sprint 6)

**Problema.** El login con Google no pasaba por el segundo factor: una cuenta con MFA activo entraba con solo Google (§5sexies lo dejó documentado como gap).

**Diseño.** El flujo OAuth no puede pedir un código a mitad de camino, así que el callback `signIn` de NextAuth, si la cuenta tiene `mfaHabilitado`, **no crea sesión**: devuelve una URL (`/login?mfa=google&p=<prueba>`, un `string` en `signIn()` es un redirect en NextAuth v4). La *prueba* (`src/lib/google-mfa.ts`) es un token HMAC-SHA256 (`NEXTAUTH_SECRET`), de **5 minutos**, que solo demuestra "Google acaba de autenticar a este usuario" y **no da acceso por sí sola**. La pantalla pide el código y lo canjea junto con la prueba en un provider de credenciales nuevo `google-mfa`, que verifica prueba + código (mismo `verificarSegundoFactor`: TOTP o código de respaldo de un solo uso, con el rate limit `mfaVerificacion`) y recién ahí crea la sesión revocable. **Sin cookies** (no se depende de que un callback pueda fijarlas). Falla cerrada: prueba vencida, alterada, o de una cuenta que ya no exige MFA → "vuelve a entrar con Google", sin sesión.
