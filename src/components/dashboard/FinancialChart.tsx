    ...
    <div className="card p-5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-4 text-[11px] text-[var(--text-secondary)]">
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-sm bg-negative-100 inline-block"></span>Gastos
            </span>
            <span className="flex items-center gap-1.5">
      ...
      <ResponsiveContainer width="100%" height={180}>
        <BarChart data={monthlyData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
          ...
          <XAxis
            dataKey="mes"
            tick={{ fontSize: 11, fill: "var(--color-text-mute)" }}
            axisLine={false}
            tickLine={false}
          />
          <YAxis
          ...
          <Bar dataKey="gastos" name="Gastos" fill="var(--color-negative)" radius={[3, 3, 0, 0]} />
          <Bar dataKey="ingresos" name="Ingresos" fill="var(--color-positive)" radius={[3, 3, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
