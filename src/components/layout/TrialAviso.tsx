 ...
 */
export function TrialAviso({ nombre, dias, vencido, esOwner, suspendida = false }: { nombre: string; dias: number | null; vencido: boolean; esOwner: boolean; suspendida?: boolean }) {
  const bloqueada = vencido || suspendida;
  if (!bloqueada && (dias === null || dias > 7)) return null; // más de 7 días: sin ruido

  const Icono = bloqueada ? Lock : Clock;
  return (
    <div
      role="status"
      className="w-full flex items-center justify-center gap-2 px-4 py-2 text-[12px] font-medium text-center"
      style={bloqueada ? { backgroundColor: "#fee2e2", color: "#991b1b" } : { backgroundColor: "var(--agro-amber)", color: "var(--agro-amber-dark)" }}
    >
      <Icono size={15} aria-hidden="true" className="shrink-0" />
      <span>
        {suspendida && !vencido
          ? `${nombre} está suspendida y en modo lectura (puedes ver todo, pero no agregar ni editar).`
          : vencido
